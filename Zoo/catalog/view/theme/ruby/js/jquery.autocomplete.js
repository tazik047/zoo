...
jQuery.fn.autocomplete = function(url, settings ) 
{
	return this.each( function()//do it for each matched element
	{
		...
		var list = valueInput.next().css({top: textInput.offset().top + textInput.outerHeight() + 19,/* left: textInput.offset().left,*/ width: 200/*textInput.width() + 50*/});
		var oldText = '';
		var typingTimeout;
		var size = 0;
		var selected = 0;
				...
				$.getJSON(url,settings.parameters,function(data)
				{
					...
					if (data)
					{
						...
						for (i = 0; i < data.length; i++)//iterate over all options
						{
						  ...
						  click(function () { valueInput.val( $(this).attr('value') );textInput.val( $(this).text() ); clear(); window.location.href = $('base').attr('href') + 'index.php?route=product/product&product_id=' + $(this).attr('value');});
